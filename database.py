...
class CsvSearcherGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.searched_names = []  # Keep track of searched names for highlighting
    ...
    def init_ui(self):
        ...
        # Map button
        map_btn = QPushButton("🗺️ Vis Kort - Byer")
        map_btn.clicked.connect(self.show_map)
        map_btn.setStyleSheet("background-color: #9C27B0; color: white; padding: 10px;")
        main_layout.addWidget(map_btn)
        
        # Group search button
        group_btn = QPushButton("👥 Søg efter Gruppe/Husstand")
        group_btn.clicked.connect(self.show_group_search)
        group_btn.setStyleSheet("background-color: #FF5722; color: white; padding: 10px;")
        main_layout.addWidget(group_btn)
    ...
    def display_results(self, results: List[Dict]):
        ...
        for result in results:
            for k, v in result.items():
                # Skip None keys (from empty column headers in CSV)
                if k is None:
                    continue
    ...
    def show_map(self):
        """Show map with cities where people lived"""
        ...
        try:
            # Extract all cities/locations from results
            cities = {}
            location_columns = ['By', 'Stilling_i_husstanden', 'Kildeerhverv', 'Bopæl', 
                               'Sogn', 'Kommune', 'Amt', 'By_navn', 'City', 'Location']
            
            # Collect cities and count occurrences
            for result in self.current_results:
                # Look for city/location info in various column names
                for col in result.keys():
                    if col is None:  # Skip None column names
                        continue
                    if any(x in col.lower() for x in ['by', 'stadt', 'city', 'sogn', 'kommune']):
                        city_name = str(result.get(col, '')).strip()
                        if city_name and len(city_name) > 1 and not city_name.startswith('['):
                            city_name = city_name.split(',')[0].strip()  # Take first part if multiple
                            cities[city_name] = cities.get(city_name, 0) + 1
                            break
            
            if not cities:
                QMessageBox.warning(self, "Fejl", 
                    "Kunne ikke finde by-oplysninger i søgeresultater.\n"
                    "Sørg for at der er en kolonne med bynavne.")
                return
            
            # Create map with geocoding
            QMessageBox.information(self, "Kort", 
                f"Opbygger kort med {len(cities)} byer...\n"
                "Dette kan tage et øjeblik.")
            
            # Initialize geocoder
            geolocator = Nominatim(user_agent="csv_searcher_dk")
            
            # Create map centered on Denmark
            map_center = [56.26, 9.50]  # Denmark center
            ...
            for city, count in sorted(cities.items(), key=lambda x: x[1], reverse=True):
                try:
                    # Geocode the city (add Denmark to search context)
                    location = geolocator.geocode(f"{city}, Danmark")
                    
                    if location:
                        # Add marker with city count
                        popup_text = f"<b>{city}</b><br>Antal personer: {count}"
                        folium.CircleMarker(
                            location=[location.latitude, location.longitude],
                            radius=min(30, max(5, count / 10)),  # Size based on count
            ...
            if successfully_plotted == 0:
                QMessageBox.warning(self, "Fejl", 
                    "Kunne ikke geocode nogle byer.\n"
                    "Kontrollér internetforbindelsen.")
                return
            
            # Save map to temporary file
            map_file = os.path.join(tempfile.gettempdir(), "census_map.html")
            m.save(map_file)
            
            # Create dialog with web view
            map_dialog = QDialog(self)
            map_dialog.setWindowTitle(f"Kort - {successfully_plotted} byer med {sum(cities.values())} personer")
            map_dialog.setGeometry(50, 50, 1200, 800)
            
            layout = QVBoxLayout()
            
            # Info label
            info_text = f"Viser {successfully_plotted} ud af {len(cities)} byer fra søgeresultater. "\
                       f"Størrelse af cirkler indikerer antal personer."
            ...
            QMessageBox.critical(self, "Fejl", f"Fejl ved opbygning af kort:\n{str(e)}")
    
    def show_group_search(self):
        """Show dialog to search for multiple people in same household"""
        dialog = QDialog(self)
        dialog.setWindowTitle("👥 Søg efter Gruppe/Husstand")
        dialog.setGeometry(100, 100, 600, 500)
        
        layout = QVBoxLayout()
        
        # Instructions
        layout.addWidget(QLabel("<h3>Søg efter flere personer (fx i samme husstand)</h3>"))
        layout.addWidget(QLabel(
            "Eksempel: Søg efter 'Kaj' og 'Ester' for at finde dem sammen i en husstand"))
        
        # Names list
        layout.addWidget(QLabel("<b>Navne at søge efter:</b>"))
        
        # Add name inputs dynamically
        name_inputs = []
        
        def add_name_field():
            field_layout = QHBoxLayout()
            name_input = QLineEdit()
            name_input.setPlaceholderText("Fornavn (obligatorisk)")
            field_layout.addWidget(name_input)
            
            remove_btn = QPushButton("Fjern")
            remove_btn.clicked.connect(lambda: remove_name_field(field_layout, remove_btn, name_input))
            field_layout.addWidget(remove_btn)
            
            scroll_layout.addLayout(field_layout)
            name_inputs.append(name_input)
        
        def remove_name_field(field_layout, remove_btn, name_input):
            while field_layout.count():
                field_layout.takeAt(0).widget().deleteLater()
            remove_btn.deleteLater()
            if name_input in name_inputs:
                name_inputs.remove(name_input)
        
        # Scroll area for names
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll_widget = QWidget()
        scroll_layout = QVBoxLayout()
        
        # Add 2 initial name fields
        add_name_field()
        add_name_field()
        ...
        add_btn = QPushButton("➕ Tilføj mere navn")
        add_btn.clicked.connect(add_name_field)
        layout.addWidget(add_btn)
        
        # Options
        layout.addWidget(QLabel("<b>Søgeindstillinger:</b>"))
        
        same_household_check = QCheckBox("Vis hele husstanden (alle søgte personer skal være til stede)")
        same_household_check.setChecked(True)
        same_household_check.setEnabled(False)  # Always enabled
        layout.addWidget(same_household_check)
        
        # Search button
        def perform_group_search():
            names = [inp.text().strip() for inp in name_inputs if inp.text().strip()]
            
            if not names:
                QMessageBox.warning(dialog, "Fejl", "Tilføj mindst ét navn!")
                return
            
            if not self.csv_folder:
                QMessageBox.warning(dialog, "Fejl", "Vælg først en mappe med CSV-filer!")
                return
            
            dialog.close()
            self._perform_group_search(names, same_household_check.isChecked())
        
        search_group_btn = QPushButton("🔍 Søg")
        search_group_btn.setStyleSheet("background-color: #4CAF50; color: white; font-weight: bold; padding: 10px;")
        search_group_btn.clicked.connect(perform_group_search)
        layout.addWidget(search_group_btn)
    ...
    def _perform_group_search(self, names: List[str], same_household: bool):
        ...
        try:
            ...
            futures = {
                executor.submit(self._search_file_for_group, csv_file, names): csv_file 
                for csv_file in csv_files
            }
            ...
            # Group by household - only show households with ALL searched names
            households = {}
            for result in all_results:
                hh_id = str(result.get('Husstands/familienr.', 'UKENDT')).strip()
                if hh_id not in households:
                    households[hh_id] = []
                households[hh_id].append(result)
            
            # Only keep households with matches for ALL search names
            filtered_results = []
            valid_households = 0

            print(f"DEBUG: all_results = {len(all_results)} persons")
            print(f"DEBUG: households = {len(households)}")
            print(f"DEBUG: search names = {names}")
            
            for hh_id, matched_members in households.items():
                # Check if ALL search names are present in this household
                member_names = [str(m.get('Kildenavn', '')).lower() for m in matched_members]
                matched_names = sum(1 for name in names 
                                   if any(name.lower() in mname for mname in member_names))
                
                # Only if ALL search names match
                if matched_names >= len(names):
                    # Add only the matched persons (not all household members)
                    for search_name in names:
                        for result in matched_members:
                            result_name = str(result.get('Kildenavn', '')).lower()
                            if search_name.lower() in result_name:
                                result_copy = result.copy()
                                result_copy['_is_search_hit'] = True
                                # DONT store the entire file - read it in dialog instead
                                if '_hele_filen' in result_copy:
                                    del result_copy['_hele_filen']
            ...
            self.progress_bar.setVisible(False)
            # Store searched names for highlighting in household dialog
            self.searched_names = names
    ...
    def _search_file_for_group(self, csv_file: Path, names: List[str]) -> List[Dict]:
        """Search a single CSV file for people with given names"""
        results = []
        try:
            ...
            for row in entire_file:
                person_name = str(row.get('Kildenavn', '')).lower()
                
                # Check if any of the search names match
                if any(name.lower() in person_name for name in names):
                    row['_fil'] = csv_file.name
                    row['_sti'] = str(csv_file)
                    row['_hele_filen'] = entire_file  # Add entire file for household lookup
                    results.append(row)
        except:
            pass
    ...
    def show_details(self):
        ...
        all_keys = sorted([k for k in person.keys() if k is not None and not k.startswith('_')])
        row_count = 0
        for key in all_keys:
        ...
        if husstands_nr and husstands_nr != 'nan' and husstands_nr != '':
            ...
            if hele_filen:
                for member in hele_filen:
                    if str(member.get('Husstands/familienr.', '')).strip() == husstands_nr:
                        # Mark if this member is a search hit (has same name as searched)
                        member_name = str(member.get('Kildenavn', '')).lower()
                        is_search_hit = False
                        if hasattr(self, 'searched_names') and self.searched_names:
                            is_search_hit = any(search_name in member_name for search_name in self.searched_names)
                        member['_is_search_hit'] = is_search_hit
            ...
            if household_members:
                ...
                for member in household_members:
                    household_cols.update([k for k in member.keys() if k is not None and not k.startswith('_')])
                ...
                for row, member in enumerate(household_members):
                    for col, field in enumerate(columns):
                        ...
                        # Highlight ONLY if this member was a search hit AND contains a searched name
                        is_search_hit = member.get('_is_search_hit', False)
                        if is_search_hit and hasattr(self, 'searched_names') and self.searched_names:
                            value_lower = value.lower()
                            for searched_name in self.searched_names:
                                if searched_name in value_lower:
                                    # Use light yellow background for highlighting
                ...
                for idx, member in enumerate(household_members):
                    ...
                    member_items = [(k, v) for k, v in member.items() if k is not None]
                    for key, value in sorted(member_items):
